...
_TypeOfWreck = [];
_typeOfUnit = [];
if (count zagor_arr_CrashSiteWrecksCar >0) then {
	_blah=floor(random 2);
	if (_blah == 0) then {
	_TypeOfWreck = zagor_arr_CrashSiteWrecks;
	_typeOfUnit = zagor_arr_CrashSiteCrew;
	} else {
	_TypeOfWreck = zagor_arr_CrashSiteWrecksCar;
	_typeOfUnit = zagor_arr_CrashSiteCrewCar;
	};
} else { 
	_TypeOfWreck = zagor_arr_CrashSiteWrecks;
	_typeOfUnit = zagor_arr_CrashSiteCrew;
...
if(isNil("zagor_CrashSiteMarkerNo")) then {
	zagor_CrashSiteMarkerNo = 0;
} else {
	zagor_CrashSiteMarkerNo = zagor_CrashSiteMarkerNo + 1;
};
["zagor_CrashSiteMarker" + str zagor_CrashSiteMarkerNo,_position,"hd_warning","ColorGreen",true] call Zagor_fnc_createLocationMarker;


//Create Smoke
private _effect = selectRandom ["SMOKE_SMALL", "SMOKE_MEDIUM", "SMOKE_BIG"];
_fx = [_position, _effect];
_fx remoteExec ["Zagor_fnc_FireSmokeFX",0,true];
...
_boxpos = _position findEmptyPosition [3,15,_boxType];
 if(count _boxpos >0) then {
    ...
    for "_i" from 0 to (count zagor_arr_CrashSiteWeapons - 1) do {
	 ...
	 for "_i" from 0 to (count zagor_arr_CrashSiteItems - 1) do {
        private ["_item", "_itemClassName", "_probabilityOfPrecence", "_minCount", "_maxCount"];
        
        _item = zagor_arr_CrashSiteItems select _i;
	...
	_grp = createGroup Zagor_VAR_Side_Blufor;
