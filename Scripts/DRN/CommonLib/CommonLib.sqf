...
zagor_var_CL_SilentDebugMode = false;
...
drn_fnc_CL_AddUnitsToGarbageCollector = {
    ...
    /*if (isNil "zagor_var_CL_GarbageCollectorUnits") then {
        zagor_var_CL_GarbageCollectorUnits = [];
    };
    
    zagor_var_CL_GarbageCollectorUnits = zagor_var_CL_GarbageCollectorUnits + _units;*/
};
 ...
 *   [Zagor_Debug]: Optional. true to enable debug mode, else false.
 */
drn_fnc_CL_RunGarbageCollector = {
    ...
    while {true} do {
       ...
       /* // Delete units that are far away
        ...
        } foreach zagor_var_CL_GarbageCollectorUnits;
        
        zagor_var_CL_GarbageCollectorUnits = zagor_var_CL_GarbageCollectorUnits - _unitsToDelete;
...
drn_CLZagor_DebugTextEventArgs = []; // Empty
drn_CLZagor_DebugMarkerEventArgs = []; // [name, position, size, direction, shape ("RECTANGLE" or "ELLIPSE"), markerColor, markerText (optional)] or alternatively [name, position, type, markerColor (optional), markerText (optional)]
drn_CL_DeleteDebugMarkerEventArgs = [];  // [name]

"drn_CLZagor_DebugTextEventArgs" addPublicVariableEventHandler {
    drn_CLZagor_DebugTextEventArgs call drn_fnc_CL_ShowDebugTextLocal;
};

"drn_CLZagor_DebugMarkerEventArgs" addPublicVariableEventHandler {
    drn_CLZagor_DebugMarkerEventArgs call drn_fnc_CL_SetDebugMarkerLocal;
};
 ...
 * Remarks:
 *   if global variable "zagor_var_CL_SilentDebugMode" is set to true, debug text will only be written to RTF-file and not shown on screen.
 * Arguments:
 *   _text: Debug text.
 */
drn_fnc_CL_ShowDebugTextLocal = {
    ...
    if (!isNull player) then {
        if (!zagor_var_CL_SilentDebugMode) then {
...
drn_fnc_CL_ShowDebugTextAllClients = {
    drn_CLZagor_DebugTextEventArgs = _this;
    publicVariable "drn_CLZagor_DebugTextEventArgs";
    drn_CLZagor_DebugTextEventArgs call drn_fnc_CL_ShowDebugTextLocal;
};
 ...
 * Remarks:
 *   if global variable "zagor_var_CL_SilentDebugMode" is set to true, debug marker will not shown.
...
drn_fnc_CL_SetDebugMarkerAllClients = {
    drn_CLZagor_DebugMarkerEventArgs = _this;
    publicVariable "drn_CLZagor_DebugMarkerEventArgs";
    drn_CLZagor_DebugMarkerEventArgs call drn_fnc_CL_SetDebugMarkerLocal;
};
...
zagor_var_CL_CommonLibVersion = 1.04;
Zagor_var_commonLibInitialized = true;
