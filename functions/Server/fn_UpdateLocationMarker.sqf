...
private _poi = (missionnamespace getvariable ["Zagor_POIs",[]] select {_x # 0 == _markername});
...
if(!_hidden && _unknown || _reveal) then {
	...
	if(_markerType == "b_hq") then { // o_hq
		//Update the task to find a comcenter marker
		missionNamespace setvariable ["Zagor_Task_LocateComcenter_Complete",true,true];
	};
	_what = _markerType;
