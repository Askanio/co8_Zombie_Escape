...
private _paramLoading = "Zagor_Param_Loadparams" call BIS_fnc_getParamValue;
switch _paramLoading do {
    case 0: {
        ["Saving parameters."] call Zagor_fnc_rptLog;
        profileNamespace setVariable ["Zagor_SavedParams", paramsArray];
        saveProfileNamespace;
    };
    case 1: {
        if (missionNamespace getVariable ["Zagor_UseCBASettings", false]) then {
            "Using CBA settings" call Zagor_fnc_rptLog;
            ...
            private _params = profileNamespace getVariable ["Zagor_SavedParams",[]];
            if(count(_params)==0 || count(_params)!=count(paramsArray)) then {
                ["No parameters found or params were updated, loading default."] call Zagor_fnc_rptLog;
            } else {
                ["Parameters loaded"] call Zagor_fnc_rptLog;
                paramsArray = _params;
                publicVariable "paramsArray";
            };
        };
    ...
    case 2: {
        "Using manual parameters." call Zagor_fnc_rptLog;
    };
};
...
_paramsBriefing remoteExec ["Zagor_fnc_WriteParamBriefing", 0, true];
diag_log _paramsBriefing;

Zagor_ParamsParsed = true;
publicVariable "Zagor_ParamsParsed";
