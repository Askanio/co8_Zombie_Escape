...
diag_log format["Start radioactive_object.sqf"];
...
if (AI_react_rad) then {[[_obj_rad_obj,_rad_radius_obj],"Scripts\AL_radiation\ai_avoid_rad.sqf"] remoteExec ["execvm",0,true];};

waitUntil {!isNil "glowindark"};

[[_obj_rad_obj,_rad_radius_obj,_rad_dam_obj], "Scripts\AL_radiation\rad_emitter.sqf"] remoteExec ["execVM",0,true];

if (glowindark) then 
{
	[[_obj_rad_obj,_rad_radius_obj], "Scripts\AL_radiation\ai_flicker.sqf"] remoteExec ["execVM",0,true];
