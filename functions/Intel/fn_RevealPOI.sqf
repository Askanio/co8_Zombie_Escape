...
private _pois = (missionnamespace getvariable ["Zagor_POIs",[]] select {_x # 4 || _x # 5});
...
for [{ _i = 0 }, { _i < _numIntel }, { _i = _i + 1 }] do { 
	...
	_markerType = [_poi # 0] call Zagor_fnc_updateLocationMarker;
};
if(_numIntel==1) then {
