...
private _category = "co8 Zombie Escape";
private _settingCategory = _category;
{
    ...
    if (count _values <= 1) then {
        ...
        if (_name == "Zagor_Param_Loadparams") then {
            _name = "Zagor_UseCBASettings";
		...
		//private _varname = [(_x),"varname","zagor_" + _name] call BIS_fnc_returnConfigEntry;
        [
            _name,
            _type,
            [_title, _tooltip],
